#!/bin/bash
#
# Usage: ./launch-mamba.sh <mode> <model_size> [steps] [nodes] [hybrid_ratio]
#
...
# Hybrid ratio: optional, default 0 (pure Mamba2).
#               E.g. 0.25 = 25% attention layers (hybrid Mamba).
#
# Examples:
#   ./launch-mamba.sh throughput 760m                    # pure Mamba2, 4 nodes
#   ./launch-mamba.sh throughput 760m 50 4 0.25          # hybrid 25%, 4 nodes
#   ./launch-mamba.sh train 760m 5000                    # pure Mamba2, 4 nodes
#   ./launch-mamba.sh train 760m 5000 4 0.25             # hybrid 25%, 4 nodes
#   ./launch-mamba.sh train 760m 5000 8                  # pure Mamba2, 8 nodes
#
# Prerequisites: run ./setup-mamba.sh once to install causal-conv1d and mamba-ssm.

set -euo pipefail

source "$(dirname "$0")/config.sh"

MODE=${1:?Usage: ./launch-mamba.sh <mode> <model_size> [steps] [nodes] [hybrid_ratio]}
MODEL_SIZE=${2:?Usage: ./launch-mamba.sh <mode> <model_size> [steps] [nodes] [hybrid_ratio]}
...
case $MODE in
    throughput)
        ...
        ;;
    train)
        TRAINING_STEPS=${3:?Usage: ./launch-mamba.sh train <model_size> <steps> [nodes] [hybrid_ratio]}
        NODES=${4:-4}
        HYBRID_RATIO=${5:-0}
        TIME=02:30:00
        EVAL_INTERVAL=1000
...
################ Model type ################
if awk "BEGIN{exit !($HYBRID_RATIO > 0)}"; then
    RATIO_INT=$(awk "BEGIN{printf \"%d\", $HYBRID_RATIO * 100}")
    MODEL_TYPE="hybrid-${RATIO_INT}"
    HYBRID_ARG="    --hybrid-attention-ratio ${HYBRID_RATIO}"
else
    MODEL_TYPE="mamba2"
    HYBRID_ARG=""
fi

JOB_NAME="${MODE}-${MODEL_SIZE}-${MODEL_TYPE}-${NODES}n"
...
EXP_NAME=${MODE}-${MODEL_SIZE}-${MODEL_TYPE}-\${SLURM_NNODES}n
